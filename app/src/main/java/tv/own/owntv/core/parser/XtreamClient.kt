...
sealed interface HlsProbe {
    ...
    data class NotServed(val reason: String) : HlsProbe
    /** No usable answer — network failure, nothing to test with, or an unexpected status. */
    data class Inconclusive(val reason: String) : HlsProbe
}
...
class XtreamClient(private val http: HttpClient) {
        ...
        return try {
            http.get(url, s.userAgent) { input ->
                ...
                if (String(head, 0, read, Charsets.UTF_8).contains("#EXTM3U")) {
                    ...
                    HlsProbe.NotServed("the server answered, but not with a playlist")
                }
            }
        } catch (c: kotlin.coroutines.cancellation.CancellationException) {
            throw c
            ...
            when (e.code) {
                ...
                404, 400 -> HlsProbe.NotServed("no HLS endpoint for this channel (HTTP ${e.code})")
                else -> HlsProbe.Inconclusive("HTTP ${e.code}")
            }
        } catch (e: Exception) {
            Log.w(TAG, "probeHls failed for source ${s.id}", e)
            HlsProbe.Inconclusive(e.message?.takeIf { it.isNotBlank() } ?: e.javaClass.simpleName)
        }
    }
    ...
    suspend fun testHlsSupport(s: SourceEntity, knownStreamId: String? = null): HlsTest {
        val details = fetchAccountDetails(s) ?: return HlsTest(declared = null, probe = HlsProbe.Inconclusive("no answer"))
        val streamId = knownStreamId?.takeIf { it.isNotBlank() } ?: firstLiveStreamId(s)
            ?: return HlsTest(details.hlsSupported, HlsProbe.Inconclusive("this playlist has no live channels to test"))
        ...
        if (probe is HlsProbe.NotServed && !probeTsPlayable(s, streamId)) {
            return HlsTest(
                ...
                HlsProbe.Inconclusive("the test channel didn't play over MPEG-TS either, so it may just be dead"),
            )
        }
