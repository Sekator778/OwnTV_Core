...
/** Internal deep links used by Android TV home rows and external launch intents. */
sealed interface TvHomeDeepLink {
    fun toUri(): Uri

    data object OpenLiveSection : TvHomeDeepLink {
    ...
    companion object {
        fun parse(uri: Uri?): TvHomeDeepLink? {
...
sealed interface TvHomeLaunch {
    data class Movie(val movie: MovieEntity, val startPositionMs: Long) : TvHomeLaunch
    data class Episode(val show: SeriesEntity, val episode: EpisodeEntity, val queue: List<EpisodeEntity>, val startPositionMs: Long) : TvHomeLaunch
    data class Series(val show: SeriesEntity) : TvHomeLaunch
    data class Live(val channel: ChannelEntity) : TvHomeLaunch
}
