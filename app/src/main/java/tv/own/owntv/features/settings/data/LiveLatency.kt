...
object LiveBuffer {
    ...
    /** Compact form for a chip, where there is no room to spell the unit out: `Off` / `10s`. */
    fun prerollLabel(secs: Int): String = if (secs <= 0) "Off" else "${secs}s"

    /**
     * Long form, used wherever the value is *chosen* or *reported*. The unit is the whole point: the
     * number is seconds of **video** to collect, not seconds to wait, and on a fast provider those ten
     * seconds arrive in well under one. Writing it as "10s of video" everywhere stops the setting from
     * reading like a countdown — the one thing users asked about first.
     */
    fun prerollValueLabel(secs: Int): String = if (secs <= 0) "Off" else "${secs}s of video"
